...
const AboutPill: React.FC = React.memo(() => (
    <div className="inline-flex items-center gap-2 bg-card dark:bg-dark-card border border-border dark:border-dark-border px-3 py-1 rounded-full text-sm text-text-muted dark:text-dark-text-muted">
...
const FeaturedLogo: React.FC<{children: React.ReactNode}> = React.memo(({ children }) => (
    <div className="text-base font-semibold text-text-muted dark:text-dark-text-muted opacity-70 hover:opacity-100 transition-opacity">
        {children}
    </div>
));

const SectionTitle: React.FC<{children: React.ReactNode}> = React.memo(({ children }) => (
    <h2 className="text-3xl md:text-4xl font-bold text-text-main dark:text-dark-text-main tracking-tight">{children}</h2>
));

const FeatureCard: React.FC<{icon: React.ReactNode; title: string; children: React.ReactNode}> = React.memo(({ icon, title, children }) => (
    <div className="bg-card dark:bg-dark-card p-6 rounded-xl border border-border dark:border-dark-border text-left space-y-3">
        <div className="flex items-center gap-3">
            <div className="bg-background dark:bg-dark-background p-2 rounded-lg border border-border dark:border-dark-border">
                {icon}
            </div>
            <h3 className="text-lg font-semibold text-text-main dark:text-dark-text-main">{title}</h3>
        </div>
        <p className="text-text-muted dark:text-dark-text-muted">{children}</p>
    </div>
));

const TeamMemberCard: React.FC<{name: string; role: string; imageUrl: string; children: React.ReactNode}> = React.memo(({ name, role, imageUrl, children }) => (
     <div className="bg-card dark:bg-dark-card p-6 rounded-xl border border-border dark:border-dark-border text-center space-y-3">
        <img src={imageUrl} alt={`Photo of ${name}`} className="w-24 h-24 rounded-full mx-auto border-2 border-border dark:border-dark-border object-cover" />
        <div>
            <h4 className="text-lg font-bold text-text-main dark:text-dark-text-main">{name}</h4>
            <p className="text-accent font-medium">{role}</p>
        </div>
        <p className="text-sm text-text-muted dark:text-dark-text-muted">{children}</p>
    </div>
));
    ...
    return (
        <div className="max-w-5xl mx-auto py-16 md:py-24 px-4 space-y-24 animate-fade-in">
            ...
            <div className="relative text-center space-y-6">
                <AboutPill />
                <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-text-main dark:text-dark-text-main">
                    Powering Smarter Trading
                </h1>
                <p className="max-w-2xl mx-auto text-lg text-text-muted dark:text-dark-text-muted">
            ...
            <div className="space-y-8 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <FeatureCard
                        icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M12 6V4m0 16v-2M12 12l-2 2-2-2m4 4l2-2 2 2" /></svg>}
                    ...
                    <FeatureCard
                        icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                <div className="space-y-4">
                    ...
                    <p className="text-text-muted dark:text-dark-text-muted text-lg leading-relaxed">
            ...
            {/* Team Section */}
             <div className="space-y-8 text-center">
                ...
                <p className="max-w-2xl mx-auto text-text-muted dark:text-dark-text-muted">
            ...
            <div className="space-y-8 text-center">
                <p className="text-sm text-text-muted dark:text-dark-text-muted tracking-widest uppercase">As featured in</p>
