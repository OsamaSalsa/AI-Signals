...
const IndicatorBox: React.FC<{title: string; children: React.ReactNode}> = React.memo(({ title, children }) => (
    <div className="flex-1 bg-background dark:bg-dark-background p-4 rounded-lg border border-border dark:border-dark-border min-w-[200px]">
        <h4 className="font-semibold text-text-muted dark:text-dark-text-muted mb-3 text-sm">{title}</h4>
        {children}
    </div>
));

const RsiDisplay: React.FC<{ rsi: Rsi }> = React.memo(({ rsi }) => {
    const getRsiStyle = (interpretation: string): string => {
        const lowerInterp = interpretation.toLowerCase();
        if (lowerInterp.includes('overbought')) {
            return 'bg-danger-soft text-danger';
        }
        if (lowerInterp.includes('oversold')) {
            return 'bg-success-soft text-success';
        }
        return 'bg-accent-soft text-accent'; // Neutral as default
    };

    const interpretationText = rsi.interpretation || 'Neutral';
    const badgeClass = getRsiStyle(interpretationText);

    return (
        <div className="flex items-center justify-between">
            <span className="text-2xl font-bold text-text-main dark:text-dark-text-main">{rsi.value?.toFixed(2)}</span>
...
const PivotPointsDisplay: React.FC<{ pivots: PivotPoints }> = React.memo(({ pivots }) => (
    <div className="space-y-1 text-sm">
        ...
        <div className="flex justify-between font-bold text-text-main dark:text-dark-text-main"><span>Pivot</span><span>{pivots.pivot}</span></div>
...
const SmaDisplay: React.FC<{ sma: Sma }> = React.memo(({ sma }) => (
    <div className="space-y-1 text-sm text-text-main dark:text-dark-text-main">
...
const TechnicalIndicators: React.FC<TechnicalIndicatorsProps> = ({ pivotPoints, rsi, sma }) => {
    return (
        <div className="bg-card dark:bg-dark-card p-4 rounded-xl border border-border dark:border-dark-border space-y-4">
            <h3 className="font-bold text-lg text-text-main dark:text-dark-text-main">Technical Indicators</h3>
...
export default React.memo(TechnicalIndicators);